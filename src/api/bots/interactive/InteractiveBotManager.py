...
class InteractiveBotManager(MadHatterBotManager):
    """
        TODO:
    ...
    def __init__(self):
        MadHatterBotManager.__init__(self)
    ...
    def monitor_bot(self):
        ...
        self.bot = self.c.customBotApi.backtest_custom_bot(
            self.bot.guid, self.ticks).result

        print(
            f'Selected {self.bot.name} bot is being monitored for changes to config.')
        ...
        while self.bot.botLogBook:
            ...
            sleep(1.5)
            self.bot = self.haas.client.customBotApi.get_custom_bot(
                self.bot.guid, 15)
        ...
        for c in range(self.limit):
            ...
            bot = self.haas.client.customBotApi.backtest_custom_bot(
                bot.guid, self.read_ticks()).result
        ...
        while True:
            ...
            if resp == 'Set Create Limit':
                self.limit = inquirer.text(
                    'Type number top of configs to create as bots at the end?').execute()
                self.write_file()
