...
class Optimize:
    ...
    def set_stoploss_range(self):
        ...
        stop = inquirer.text(
            message="Write stoploss range ending number: ").execute()
        step = inquirer.text(
            message="Write stoploss range stepping number: ").execute()
    ...
    def backtest_bbands_length(self):
        ...
        for i in range(len(configs.index)):
            ...
            # 		)
            print(
                f"{bot.name} bBands Length set to {int(configs.bbl.iloc[i])}")
