...
class StreamlitHaasTool(Haas):
	...
	def get_marketdata(self):
		if self.bot:
			self.interval = self.bot.interval
		marketdata = md().get_market_data(self.market,self.interval,
		                                  int(self.ticks / self.interval))
		try:
			marketdata.set_index(marketdata.Date,inplace=True)
		except Exception as e:
			print(e)
		self.marketdata = marketdata
		plot = self.plot_market_data(self.marketdata)
	...
	def market_data_viewer(self):
		...
		if self.marketdata is not None:
			...
			print('Saving...')
			d = st.empty()
			filename = f'./MarketData/{self.market.primaryCurrency}_' \
			           f'{self.market.secondaryCurrency}_{EnumPriceSource(self.market.priceSource).name}.csv'
			try:
				self.marketdata.to_csv(
					filename
					)
			...
			d = st.write(f'Successfully saved market history into {filename} file')
...
if __name__ == "__main__":
	...
	if resp == 'Twitter Data':
		tw_search = st.sidebar.text_input(label='Search for')
		days = st.sidebar.text_input(label='Days')
		tweets_limit = st.sidebar.text_input(label='Tweets Limit')
		btn = st.sidebar.button('Search')
		if btn:
			df = Twitter_Parser(limit=int(tweets_limit),search_phrase=tw_search,for_days=int(days))
			df = df.search()
			if len(df.index)>0:
				items = []
				for i in range(len(df.index)):
					items.append(df.full_text[i])
				st.write(items)
			else:
				st.write('No results have been found, please change the search request')
	elif resp == 'Plot Bot Trades':
