...
class Haas():
    ...
    def write_date(self):
        ...
        choices = [f'Write Year:', f'Write month (current is {str(datetime.datetime.today().month)}): ',
                   f'Write day (today is {str(datetime.datetime.today().day)}: ',
                   f'Write  hour (now is {str(datetime.datetime.today().hour)}):',
                   f'Write min ( mins now {str(datetime.datetime.today().minute)}): ']
    ...
    def configure_haas(self):
        ...
        if not setup['apiConfigured']:
            print(
                f'Following is a step by step guide to configure Haas Local Api. After doing each step, type Y on keyboard to get to next step. \n')
            config_questions = [
                ...
                inquirer.Text(
                    'inputIP',
                    'Input IP. If this app on the same server type:127.0.0.1, else: server address that you use to access Haas remotely'),
                ...
                inquirer.Text(
                    'inputSecret',
                    'Input API key in the next field. If app running on the same server, can be simple.  Else, make it complicated'),
                ...
                inquirer.Text(
                    'passwordSavedAgain',
                    'Once Haas done its thing, go back to Settings > Local API page and check that password field is not empty. If empty, enter it again and hit save again'),

                inquirer.Text('restarted',
                              'Once saved, restart Haas server. If running on windows, check if there is any mentions of Local API server in the console app. App will test the connection itself at a later stage'),
                inquirer.Text(
                    'apiActivated',
                    'API should be activated, Now lets input API data into the app. This only needs to be done once.'),
...
class Main_Menu(Haas):
    ...
    def main_screen(self):
        ...
        # os.system('clear')
        questions = [
            inquirer.List('resp', 'Select an option using keyboard up and down keys, then hit Return : ',
                          choices=choices)]
    ...
    def tw_to_bots(self, bottype, file=None):
        ...
        for bot in botlist2:
            if bot not in botlist:
                if EnumBot == 15:
                    ...
                    configs = BotDB().setup_bot_from_csv(
                        self.bot, configs.iloc[int(resp)])
                    # print(Haas().read_ticks)
                    bot = BotDB().bt_bot(self.bot, Haas().read_ticks())
                    newbots.append(bot)
                elif EnumBot == 3:
    ...
    def create_bots_from_tradingview_screener(self, EnumBot, market_object, account):
        ...
        newbot = self.c.customBotApi.new_custom_bot(
            account.guid, EnumBot, f'TW {market_object.primaryCurrency}/{market_object.secondaryCurrency}',
            market_object.primaryCurrency, market_object.secondaryCurrency, market_object.contractName).result
        return newbot
    ...
    def multiple_bot_auto_bt_menu(self):
        ...
        menu = [inquirer.List('response', message='Please chose an action:', choices=[
            'Select Bots', 'Select config file', 'Set configs limit', 'Set create limit', 'Start Backtesting',
            'Main Menu'])]
...
class MadHatterBot(Bot):
    ...
    def trades_to_df(self, bot):
        if len(bot.completedOrders) > 0:
            completedOrders = [
                {'orderId': x.orderId, 'orderStatus': x.orderStatus, 'orderType': x.orderType, 'price': x.price,
                 'amount': x.amount,
                 'amountFilled': x.amountFilled, 'date': pd.to_datetime(x.unixAddedTime, unit='s')} for x in
                bot.completedOrders]
            ...
            completedOrders = [{'orderId': None, 'orderStatus': None, 'orderType': None, 'price': None,
                                'amount': None, 'amountFilled': None, 'unixTimeStamp': datetime.datetime.today().year()}
                               for x in range(1)]
            orders_df = pd.DataFrame(completedOrders)
        return orders_df
...
class ScalperBotClass(Bot):
    ...
    def setup_scalper_bot(self, bot, targetpercentage, safetythreshold):
        ...
        do = self.c.customBotApi.setup_scalper_bot(accountguid=bot.accountId, botguid=bot.guid, botname=bot.name,
                                                   primarycoin=bot.priceMarket.primaryCurrency,
                                                   secondarycoin=bot.priceMarket.secondaryCurrency,
                                                   templateguid=bot.customTemplate,
                                                   contractname=bot.priceMarket.contractName, leverage=bot.leverage,
                                                   amountType=bot.amountType, tradeamount=1000,
                                                   position=bot.coinPosition, fee=bot.currentFeePercentage,
                                                   targetpercentage=targetpercentage, safetythreshold=safetythreshold)
            ...
            with alive_bar(len(self.bot)) as bar:
                for bot in self.bot:
                    if len(bot.completedOrders) > 0:
                        ...
                        for s in tqdm(np.arange(float(self.safetythreshold[0]), float(self.safetythreshold[1]),
                                                float(self.safetythreshold[2]))):

                            for t in tqdm(np.arange(float(self.targetpercentage[0]), float(self.targetpercentage[1]),
    ...
    def scalper_bot_menu(self):
        # choices =
        menu = [inquirer.List('response', message='Please chose an action:', choices=[
            'Select bots', 'Set range for safety threshold', 'Set range for target percentage', 'Backtest',
            'backtest every bot', 'Main menu'])]
...
class TradeBot(Bot):
    ...
    def setup_indicator(self, bot, indicator):
        setup = self.c.TradeBotApi.setup_indicator(bot.guid, indicator.guid,
                                                   bot.priceMarket, bot.priceMarket.primaryCurrency,
                                                   bot.priceMarket.secondaryCurrency, bot.priceMarket.contractName,
                                                   indicator.timer, indicator.chartType, indicator.deviation)
        print(
...
class MarketData(Haas):
    ...
    def return_priceMarket_object(self, pricesource: EnumPriceSource, primarycoin=None, secondarycoin=None,
                                  ticker=None):
        '''
...
class InteractiveBT(Bot):
    def __init__(self):
        ...
        self.indicator_vars = ['guid', 'RsiLength', 'RsiOversold', 'RsiOverbought', 'MacdFast', 'MacdSlow', 'MacdSign',
                               'Length', 'Devup', 'Devdn', 'MaType', 'Deviation', 'ResetMid', 'AllowMidSell',
                               'RequireFcc', 'Interval']
    ...
    def compare_indicators(self, bot, bot2):
        ...
        for i in indicators:
            for ii in i:
                for v in ['RsiLength', 'RsiOversold', 'RsiOverbought', 'MacdFast', 'MacdSlow', 'MacdSign', 'Length',
                          'Devup', 'Devdn', 'MaType', 'Deviation', 'ResetMid', 'AllowMidSell', 'RequireFcc',
                          'Interval']:
                    try:
