...
class MadHatterBot(Haas,Optimize):
    ...
    def iterate_csv(self,configs,bot):
        try:
            ...
            with alive_bar(
                len(configs.index),title=f"{self.bot.name} backtesting. "
                ) as bar:
                ...
                for i in configs.index:
                    try:
                        ...
                        print(
                            configs.sort_values(by="roi",ascending=False).drop(
                                "obj",axis=1
                                )[0:10]
                            )
                    except:
    ...
    def calculate_average_trade(self):
        # orderbook_storage = []
        # orderbook_data = self.c.marketDataApi.get_order_book_from_market(
        # self.bot.priceMarket).result
        last_trades_storage = []
