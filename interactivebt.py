...
class InteractiveBT(Haas):
		def __init__(self):
				Haas.__init__(self)
				self.indicator_vars = self.get_indicator_vars()
				self.indicators = ["rsi","bBands","macd"]
				self.interval = ["Interval","interval"]
				self.ticks = Haas().read_ticks()
		
		def get_indicator_vars(self):
				return [ "guid",
						"RsiLength",
						"RsiOversold",
						"RsiOverbought",
		...
		def print_user_message(self):
				...
				print("")
				print(
						"InteractiveBT, or AssistedBT implies that you are manually changing bot parameters while backtesting is "
            "triggered automatically at a given interval or by a bot parameter change"
						)
				print(
						"Every BT session, is saved on your drive. Session ends if the ROI of the last 10 backtests was exactly "
            "the same. Every config from it can be applied to any other MH bot afterwards"
						)
				print(
						"Open any Mad-Hatter bot in FULL SCREEN, open BOT REMOTE to instantly see backtestin results, navigate to "
            "indicators tab, click on any parameter value. Now, with keyaboard arrow keys change the value up and down."
						)
				print(
						"TAB and SHIFT+TAB keys, provide you with immense speed of indicators navigation!"
						)
				print(
						"As long as typing coursor able to edit indicator values now, pressing TAB will move it to the next "
            "numeric parameter above. SHIFT+TAB moves it down."
						)
				...
				print(
						"To begin the process, simply chose Mad-Hatter Bot and change a few params here and there with the above "
            "written method!"
						)
