...
class TradingViewManager(Haas):
    ...
    def process_csv(self):
        ...
        selected_csv = inquirer.select(
            message="Select config file with markets to create:", choices=csvs
        ).execute()
    ...
    def setup_ping_pong_bot(self, bot):
        if EnumCustomBotType[self.bottype].value == 2:
            bot = self.client.customBotApi.setup_ping_pong_bot(self.exchange.guid, bot.guid, bot.name,
                                                               bot.priceMarket.primaryCurrency,
                                                               bot.priceMarket.secondaryCurrency, bot.customTemplate,
                                                               bot.priceMarket.contractName, bot.leverage,
                                                               bot.amountType,
                                                               bot.priceMarket.minimumTradeAmout if bot.currentTradeAmount < bot.priceMarket.minimumTradeAmout else bot.currentTradeAmount,
                                                               0, bot.priceMarket.fee, bot)
