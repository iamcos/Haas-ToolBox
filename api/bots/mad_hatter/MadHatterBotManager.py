...
class MadHatterBotManager(Optimize, FineTune, Menus):
    ...
    def get_mad_hatter_bots(self) -> list[BaseCustomBot]:
    ...
    def _set_safeties(self, bot: Any) -> None:
        safeties_config: dict[EnumMadHatterSafeties,
                              int] = self._generate_safeties_config()
    ...
    def read_limits(self):
        prefix: str = '_cfg_'
        cfg_funcs_chain: list[str] = [
            entry[0] for entry in MadHatterBotManager.__dict__.items() if prefix in entry[0]]
    ...
    def get_market_data(self):
        market_data = MarketData().get_market_data(
            self.bot.priceMarket, self.bot.interval, int(
                self.ticks) / self.bot.interval
        )
        return market_data
