...
class MainMenu(Haas):
	...
	def dev_features(self):
		question = [
			inquirer.List(
				"resp",
				"Select Something",
				["Create Scalper bots from Tradingview CSV file", "Main Menu"],
			)
		]
	...
	def multiple_bot_auto_bt_menu(self):
		...
		while True:
			...
			if user_response == "Select Bots":
				...
				create_limit = [
					inquirer.Text("step", message="Type how many top bots to create ")
				]
	...
	def bt(self):
		...
		for b in self.bot:
			...
			bt_results.reset_index(inplace=True)
			bt_results.to_csv(filename)
			name = f""
			if self.limit:
				if len(self.bot) < self.limit:
					self.limit = len(self.bot)
...
class FlashCrashBot(Haas):
	...
	def setup_flash_crash_bot(self, bot, targetpercentage, safetythreshold):
		...
		do = self.c.customBotApi.setup_flash_crash_bot(
			...
			EnumFlashSpreadOptions = bot.EnumFlashSpreadOptions,
			...
			safetymovein = bot.safetymovein,
			safetymoveout = bot.safetymoveout,
