...
class Haas:
    ...
    def check_config(self):
        ...
        if not os.path.exists("config.ini"):
            ...
            # print('printing CONFIG',self.config)
            #
            #
            # # Get the list of sections
            # print('printing CONFIG',self.config.sections())
...
class MainMenu(Haas):
    ...
    class MainMenu(Haas):
        ...
        def dev_features(self):
            ...
            if answer["resp"] == "Create Scalper bots from Tradingview CSV file":
                ...
                fcb = FlashCrashBot()
                print(fcb)
                self.bot = fcb.bot_selector()
                for b in self.bot:
                    fcb.setup_flash_crash_bot(b)
        ...
        def format_tw_csv(self):
            ...
            tw2 = tw[["Ticker", "Exchange"]]
            tw2 = tw2.drop_duplicates()
            priceSources = []
            for i in tw2.Exchange:
                try:
                    ...
                    priceSources.append("Unsupported")
        ...
        def bt(self):
            ...
            for b in self.bot:
                ...
                configs.reset_index(inplace=True, drop=True)
                print('index', configs.index)
                print('the configs', configs)
            ...
            if self.limit > 0:
                for c in range(self.limit):
                    ...
                    try:
                        name = f"{b.priceMarket.primaryCurrency}/{b.priceMarket.secondaryCurrency}#{c}"
                    except Exception as e:
                        name = f"{b.name}#{c}"
                        print('1013',e)
                    ...
                    new_bot = self.client.customBotApi.clone_custom_bot(
                        accountguid=b.accountId,
                        botguid=b.guid,
                        botname=b.name,
                        bottype=b.botType,
                        primarycoin=b.priceMarket.primaryCurrency,
                        secondarycoin=b.priceMarket.secondaryCurrency,
                    ...
                    try:
                        print("new_bot", new_bot.name)
                        print("newbot", new_bot.errorMessage, new_bot.errorCode)
                        print("new_bot", new_bot)
                    except Exception as e:
                        print('1845',e)
                    print(bt_results.iloc[c])
                    BotDB().setup_bot_from_csv(new_bot, bt_results.iloc[c])
                    BotDB().bt_bot(new_bot, Haas().read_ticks())
    ...
    class InteractiveBT(Haas):
        ...
        def backtest(self, loop_count=1):
            ...
            while True:
                ...
                print(
                    ...
                    bot2.roi,
                    "Best ROI:",
                    sorted(sat, key=lambda x: x.roi, reverse=True)[0].roi,
                )
                print("SAME_ROI Loop Count: ", loops)
                print("AutoBT will stop on SAME_ROI Loop Count: ", loop_count)
                print("Best Config: ")
                best_config = MadHatterBot().bot_config(
                    sorted(sat, key=lambda x: x.roi, reverse=True)[0]
                )
                print(best_config)
        ...
        def print_user_message(self):
            ...
            print("")
            print(
                "InteractiveBT, or AssistedBT implies that you are manually changing bot parameters while backtesting is triggered automatically at a given interval or by a bot parameter change"
            )
            print(
                "Every BT session, is saved on your drive. Session ends if the ROI of the last 10 backtests was exactly the same. Every config from it can be applied to any other MH bot afterwards"
            )
            print(
                "Open any Mad-Hatter bot in FULL SCREEN, open BOT REMOTE to instantly see backtestin results, navigate to indicators tab, click on any parameter value. Now, with keyaboard arrow keys change the value up and down."
            )
            print(
                "TAB and SHIFT+TAB keys, provide you with immense speed of indicators navigation!"
            )
            print(
                "As long as typing coursor able to edit indicator values now, pressing TAB will move it to the next numeric parameter above. SHIFT+TAB moves it down."
            )
            ...
            print(
                "To begin the process, simply chose Mad-Hatter Bot and change a few params here and there with the above written method!"
            )
    ...
    class BotDB(Haas):
        ...
        def setup_bot_from_obj(self, bot, config):
            ...
            if bot.interval != config.interval:
                setup_bot_from_obj = self.c.customBotApi.setup_mad_hatter_bot(
    ...
    class FlashCrashBot(Haas):
        ...
        def return_flash_crash_bots(self):
            bl = self.c.customBotApi.get_all_custom_bots().result
            botlist = [x for x in bl if x.botType == 6]
            return botlist

        def bot_selector(self):
            bots = self.return_flash_crash_bots()
            b2 = [
                (
                    f"{i.name} {i.priceMarket.primaryCurrency}-{i.priceMarket.secondaryCurrency}, {i.roi}",
                    i,
            ...
            question = [
                inquirer.Checkbox(
                    "bots",
                    message="Select one or more bots using spacebar. \n"
                            "Once satisfied, press Enter to continue",
                    choices=b2,
                )
            ]
            selection = inquirer.prompt(question)
        ...
        def setup_flash_crash_bot(self, bot):
            ...
            if EnumFlashSpreadOptions(bot.priceSpreadType).name == "FIXED_AMOUNT":
                # pricespread = None
                # percentageboost = bot.percentageBoost

                pass
            if EnumFlashSpreadOptions(bot.priceSpreadType).name == "PERCENTAGE":
                # pricespread = None
                # percentageboost = bot.percentageBoost
                pass
            if EnumFlashSpreadOptions(bot.priceSpreadType).name == "PERCENTAGE_WITH_BOOST":
                # pricespread = None
                # percentageboost = bot.percentageBoost
                pass
            if EnumFlashSpreadOptions(bot.priceSpreadType).name == "EXPONENTIAL":
                pass
            ...
            do = self.c.customBotApi.setup_flash_crash_bot(
                ...
                fee=bot.currentFeePercentage,
                baseprice=baseprice,
                pricespreadtype=EnumFlashSpreadOptions(priceSpreadType),
            ...
            # bt = c.customBotApi.backtest_custom_bot(self.bot.guid,self.ticks)
            print('result: ', bt.errorCode, bt.errorMessage)
            # print(bot.__dict__)
    ...
    def setup_bot(self, bot, EnumBot):
...
class FlashCrashBot(Haas):
    ...
    def bot_selector(self):
        ...
        question = [
            inquirer.List(
                "bots",
                message="Select a single bot using Enter. \n",
        ...
        try:
            ...
            print(e, "No bot has been selected, you must select one")
            bots = self.bot_selector()

        return selection["bots"]


    def setup_flash_crash_bot(self, bot):
        # print('PRICESOURCE',EnumPriceSource(bot.priceMarket.priceSource).name)

        # price = self.c.marketDataApi.get_price_ticker(EnumPriceSource(bot.priceMarket.priceSource),
        #                                                                bot.priceMarket.primaryCurrency,
        #                                               bot.priceMarket.secondaryCurrency,)
        # print('price',price.errorCode, price.errorMessage)
        ...
        # print(str(EnumFlashSpreadOptions(bot.priceSpreadType).name).capitalize(),'should be in caps')
        if EnumFlashSpreadOptions(bot.priceSpreadType).name == "Fixed_amount":
            if self.pricespread:
                for p in arange(float(self.pricespread[0]),float(self.pricespread[1]),float(self.pricespread[2])):
                    ...
                    time.sleep(2)
                    print('bt%',bt.result.roi)
                    try:
                      bt_results.append([bt.result.totalProfits,p])
        ...
        #     maxpercentage=maxpercentage,
        # )
        print('result2: ', do.errorCode, do.errorMessage)
        # print(bot.__dict__)
    ...
    def set_price_spread_range(self):
        ...
        print(self.pricespread,self.bot)
    def fcb_menu(self):
        menu_items = []
        if not self.bot or self.bot.botType != '6':
                menu_items.append('Select FCB Bot')
            ...
            if self.pricespread:
                pricespread = self.pricespread
            menu_items.append(f'Bot {self.bot.name} Selected')
            if EnumFlashSpreadOptions(self.bot.priceSpreadType).name == "FIXED_AMOUNT":
                menu_items.append('Set Price Spread Range')

        menu_items.append('Backtest')
        menu_items.append('Main Menu')
        while True:
            try:
                menu = [inquirer.List('resp',message = f'Bot {self.bot.name} Selected',choices = [
                    'Set Price Spread Range','Backtest','Main Menu'])]
            except Exception as e:
                print(e,'eexcept')
                menu = [inquirer.List('resp','Chose smthging',['Select Bot','Set Price Spread Range','Main Menu'])]

            resp = inquirer.prompt(menu)

            if resp['resp'] == 'Select Bot':
                self.bot_selector()
            if resp['resp'] == 'Set Price Spread Range':
                self.set_price_spread_range()

            if resp['resp'] == "Backtest":
                self.setup_flash_crash_bot(self.bot)
            if resp['resp'] == "Main Menu":
                break
...
def figuring_futures():
    ...
    for i in ks:
        ...
        # print(mm.c.accountDataApi.get_account_details(i).result.__dict__)
